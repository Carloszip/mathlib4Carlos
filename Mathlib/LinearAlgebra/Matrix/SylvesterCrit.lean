...
variable {m n : ℕ}
variable {R : Type v} [PartialOrder R] [RCLike R]
    ...
    · intro x hx -- not so easy part [not_done]
      ...
      let x2: Fin n →₀ R := -- and finally the 'vector'
        { support := x2sup
          ...
          mem_support_toFun := by -- hideous junk
            ...
            intro a
            by_cases H: a.val < i -- if a is less than i then it comes from the original x,
            -- else it is 0
            · let a': Fin i := {
                val := a.val
                isLt := by exact H
            ...
            · push_neg at H -- this probably could be clearer [not_done]
              have h1 : (a ∉ x2sup) := by
                ...
                intro j hj
                push_neg
                have : (finCast i j).val < i := by
                  simp [finCast]
                grw[H] at this
                exact Ne.symm (Fin.ne_of_gt this)
              have h2 : x2fun a  = 0 := by
                simp only [Fin.val_fin_lt, dite_eq_right_iff, x2fun]
                intro h
                rw [Fin.val_fin_le] at H
                grw[H] at h
                rw [lt_self_iff_false a] at h
                exact False.elim h
      ...
      have : (x.sum fun j xj ↦ x.sum fun k xk ↦ star xj * M.leadingMinor i j k * xk) =
        (x2.sum fun j xj ↦ x2.sum fun k xk ↦ star xj * M j k * xk) := by
          ...
          let f : Fin i ↪ Fin n := { toFun := finCast i -- this should probably be outside
                                     inj' := by exact
          ...
          unfold x2 x2fun
          simp only [Fin.val_fin_lt, Finsupp.coe_mk, mul_dite, mul_zero, sum_map]
          congr
          ext j
          congr
          ext k
          ...
          · rw [dif_pos H1]
            ...
            · push_neg at H2 -- contradiction
              have h1 : i.val ≤ (f j).val := H2
              have h2 : (f j).val = j.val := rfl
              have h3 : j.val < i.val := j.isLt
              linarith
          · push_neg at H1 -- contradiction
            have h1 : i.val ≤ (f k).val := H1
            have h2 : (f k).val = k.val := rfl
            have h3 : k.val < i.val := k.isLt
            linarith
      rw[this]
      exact hM.2 hx2
  exact this.det_pos -- posdef matrices have positive determinant [not_done]
